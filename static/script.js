...
function setUploadStatus(label = "No file loaded") {
...
async function handleSummarize() {
    ...
    try {
        ...
        if (currentFileName) {
            ...
            payload.append("length", currentLength);
...
async function copyToClipboard() {
    ...
    try {
        ...
        statusText.textContent = "Clipboard access is blocked in this browser.";
    }
}
...
function handleFileUpload(event) {
    ...
    if (!file.name.toLowerCase().endsWith(".txt")) {
        statusText.textContent = "Please upload a plain .txt file.";
        fileInput.value = "";
        setUploadStatus();
        return;
    }
    ...
    if (file.size > 1024 * 1024) {
        statusText.textContent = "Please keep uploaded text files under 1 MB.";
        fileInput.value = "";
        setUploadStatus();
        return;
    }
...
async function downloadSummaryPdf() {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error("The PDF could not be created right now.");
        }
